...
async def get_documents(
    ...
    """
    현재 로그인된 사용자의 모든 문서 목록을 조회합니다. (업로드 시간 내림차순)
...
@router.get(
    "/ascending",
    response_model=List[DocumentListResponse],
    summary="문서 목록 조회 (올림차순)"
)
async def get_documents_ascending(
    user_id: int = Depends(get_current_user_id),
    document_service: DocumentService = Depends(get_document_service)
):
    """
    현재 로그인된 사용자의 모든 문서 목록을 조회합니다. (업로드 시간 올림차순)
    ...
    Returns:
        List[DocumentListResponse]: 문서 메타데이터 목록
    """
    documents = await document_service.get_user_documents_ascending(user_id)
...
async def get_documents_paginated(
    ...
    """
    현재 로그인된 사용자의 문서 목록을 페이징하여 조회합니다. (업로드 시간 내림차순)
...
@router.get(
    "/paginated/ascending",
    response_model=PaginatedDocumentListResponse,
    summary="문서 목록 조회 (페이징, 올림차순)"
)
async def get_documents_paginated_ascending(
    page: int = 1,
    page_size: int = 10,
    user_id: int = Depends(get_current_user_id),
    document_service: DocumentService = Depends(get_document_service)
    ...
    """
    현재 로그인된 사용자의 문서 목록을 페이징하여 조회합니다. (업로드 시간 올림차순)
    ...
    Returns:
        PaginatedDocumentListResponse: 페이징된 문서 메타데이터 목록
    """
    result = await document_service.get_user_documents_paginated_ascending(
        user_id=user_id,
        page=page,
        page_size=page_size
    )
